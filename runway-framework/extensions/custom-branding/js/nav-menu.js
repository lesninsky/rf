...
(function($) {
	...
	var api = wpNavMenu = {
		...
		options: {
			...
			// Do not use directly. Use depthToPx and pxToDepth instead.
			globalMaxDepth: 11
		},
		...
		jQueryExtensions: function() {
			...
			$.fn.extend({
				...
				updateDepthClass: function(current, prev) {
					...
					if($(".ui-draggable-dragging, .ui-sortable-helper").hasClass('menu-spacer-template')) {
						current = 0;
						$(".sortable-placeholder").removeClass('menu-item-depth-1').addClass('menu-item-depth-0');
						return false;
					}

					/*
					if(
						$(".ui-draggable-dragging").hasClass('menu-spacer-template') || 
						$(".ui-draggable-dragging").prev().hasClass('menu-spacer-template') ||
						prev == 0)
						current = 0;    */
					...
					return this.each(function() {
						...
						if(!t.hasClass('menu-spacer-template')) {
							prev = prev || t.menuItemDepth();

							$(this).removeClass('menu-item-depth-' + prev).addClass('menu-item-depth-' + current);
						} else {
							var next = t.next();
							if(next.hasClass('menu-item-depth-1')) next.removeClass('menu-item-depth-1').addClass('menu-item-depth-0');
						}
					});
				},
				shiftDepthClass: function(change) {
					return this.each(function() {
						...
						if(depth < 1) {
							$(this).removeClass('menu-item-depth-' + depth).addClass('menu-item-depth-' + (depth + change));
						}
					});
				},
				...
				addSelectedToMenu: function(processMethod) {
					...
					return this.each(function() {
						...
						$(checkboxes).each(function() {
							...
							if(this.className && -1 != this.className.indexOf('add-to-top')) processMethod = api.addMenuItemToTop;
				...
				getItemData: function(itemType, id) {
					...
					var itemData = {},
						i, fields = ['menu-item-db-id', 'menu-item-object-id', 'menu-item-object', 'menu-item-parent-id', 'menu-item-position', 'menu-item-type', 'menu-item-title', 'menu-item-url', 'menu-item-description', 'menu-item-attr-title', 'menu-item-target', 'menu-item-classes', 'menu-item-xfn'];
					...
					this.find('input').each(function() {
						...
						while(i--) {
							...
							else if(itemType == 'add-menu-item') field = 'menu-item[' + id + '][' + fields[i] + ']';

							if(
							this.name && field == this.name) {
		...
		initToggles: function() {
			// init postboxes                       
			if(typeof postboxes == 'undefined') return false;
		...
		initSortables: function() {
			var currentDepth = 0,
				originalDepth, minDepth, maxDepth, prev, next, prevBottom, nextThreshold, helperHeight, transport, menuEdge = api.menuList.offset().left,
				body = $('body'),
				maxChildDepth, menuMaxDepth = initialMenuMaxDepth();

			// Use the right edge if RTL.
			menuEdge += api.isRTL ? api.menuList.width() : 0;

			api.menuList.sortable({
				items: 'li:not([data-is_protected="true"])',
				cancel: 'li[data-is_protected="true"]',
				...
				stop: function(e, ui) {
					...
					// If prev element is spacer set depth in 0
					if(ui.item.hasClass('menu-item-template')) ui.item.removeClass('menu-item-template');

					if(ui.item.hasClass('ui-draggable')) ui.item.removeClass('ui-draggable');

					if(ui.item.prev().hasClass("menu-spacer-template")) {
					...
					if(ui.item.data('id') == ''){
						// console.log(ui.item);
						ui.item.find('.item-controls').css('display', '');						
						count_items = $('.menu-item').length + 1;
						ui.item.data('id', 'admin-bar-menu-item-'+count_items);
						ui.item.find('.edit-item-dialog').attr('id', 'admin-bar-menu-item-'+count_items);
						ui.item.find('.item-edit').data('id', 'admin-bar-menu-item-'+count_items);
						ui.item.find('#edit-item-btn').data('id', 'admin-bar-menu-item-'+count_items);
						console.log(ui.item.data('id'));
					}

					// Change parent
					if(ui.item.hasClass('menu-item-depth-0')){
						ui.item.data('parent', '');
					}
					else if(ui.item.hasClass('menu-item-depth-1')){
						ui.item.data('parent', ui.item.prevAll('.menu-item-depth-0').data('id'));
					}					
				...
				change: function(e, ui) {
					...
					if(!ui.placeholder.parent().hasClass('menu')) if(prev !== undefined)
			...
			function updateSharedVars(ui) {
				...
				if(prev.length) maxDepth = ((depth = prev.menuItemDepth() + 1) > api.options.globalMaxDepth) ? api.options.globalMaxDepth : depth;
				else maxDepth = 0;
			}
		...
		setupInputWithDefaultTitle: function() {
			...
			$('.' + name).each(function() {
				...
				var $t = $(this);
				if($t.val() == $t.data(name)) $t.val('').removeClass(name);
			}).blur(function() {
				var $t = $(this);
				if('' == $t.val()) $t.addClass(name).val($t.data(name));
			});
		},
		...
		updateQuickSearchResults: function(input) {
			var panel, params, minSearchLength = 2,
				q = input.val();
		...
		addItemToMenu: function(menuItem, processMethod, callback) {
			...
			$.post(ajaxurl, params, function(menuMarkup) {
				...
				if(!ins.hasClass('menu-instructions-inactive') && ins.siblings().length) ins.addClass('menu-instructions-inactive');
				callback();
			});
		},
		...
		attachTabsPanelListeners: function() {
			$('#menu-settings-column').bind('click', function(e) {
				var selectAreaMatch, panelId, wrapper, items, target = $(e.target);

				if(target.hasClass('nav-tab-link')) {
					panelId = /#(.*)$/.exec(e.target.href);
					if(panelId && panelId[1]) panelId = panelId[1]
					else return false;
					...
					if(selectAreaMatch && selectAreaMatch[1]) {
						...
						if(items.length === items.filter(':checked').length) items.removeAttr('checked');
					...
					if(e.target.id && 'submit-customlinkdiv' == e.target.id) api.addCustomLink(api.addMenuItemToBottom);
					else if(e.target.id && -1 != e.target.id.indexOf('submit-')) $('#' + e.target.id.replace(/submit-/, '')).addSelectedToMenu(api.addMenuItemToBottom);
					return false;
				} else if(target.hasClass('page-numbers')) {
					$.post(ajaxurl, e.target.href.replace(/.*\?/, '').replace(/action=([^&]*)/, '') + '&action=menu-get-metabox', function(resp) {
						if(-1 == resp.indexOf('replace-id')) return;
						...
						if(!metaBoxData['markup'] || !toReplace) return;
		...
		initTabManager: function() {
			var fixed = $('.nav-tabs-wrapper'),
				...
				tabsWidth = 0,
				fixedRight, fixedLeft, arrowLeft, arrowRight, resizeTimer, css = {},
			...
			api.refreshMenuTabs = function(savePosition) {
				...
				if(!savePosition) active.makeTabVisible();
				...
				if(fixedWidth > tabsWidth) arrowLeft.add(arrowRight).hide();
				else arrowLeft.add(arrowRight).show();
			}
			...
			$.fn.extend({
				makeTabVisible: function() {
					...
					if(right > fixedRight) shift = fixedRight - right;
					else if(left < fixedLeft) shift = fixedLeft - left;
			...
			$.each([{
				...
				this.arrow.mousedown(function() {
					...
					if("-=" == that.operator) shift = Math.abs(tabsWidth - fixed.width()) - marginFluidVal;
		...
		eventOnClickEditLink: function(clickedEl) {
			var settings, item, matchedSection = /#(.*)$/.exec(clickedEl.href);
		...
		processQuickSearchQueryResponse: function(resp, req, panel) {
			...
			$items.each(function() {
				...
				if(matched && matched[1]) {
					...
					takenIDs[newID] = true;
					if(newID != matched[1]) {
						$item.html($item.html().replace(new RegExp('menu-item\\[' + matched[1] + '\\]', 'g'), 'menu-item[' + newID + ']'));
					}
				}
			});
		...
		removeMenuItem: function(el) {
			...
			el.addClass('deleting').animate({
				...
				if(!ins.siblings().length) ins.removeClass('menu-instructions-inactive');
			});
		},
