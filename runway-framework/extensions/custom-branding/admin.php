<div class="nav-tabs">
	<h2 class="nav-tab-wrapper tab-controlls" style="padding-top: 9px;">	
		<a href="<?php echo $this->self_url(); ?>" class="nav-tab <?php if($this->navigation == '') {echo "nav-tab-active";} ?>">Admin Bar</a>
		<a href="<?php echo $this->self_url('login-register'); ?>" class="nav-tab <?php if($this->navigation == 'login-register') {echo "nav-tab-active";} ?>">Login/Register</a>
		<a href="<?php echo $this->self_url('footer'); ?>" class="nav-tab <?php if($this->navigation == 'footer') {echo "nav-tab-active";} ?>">Footer</a>
	</h2>
</div>
...
switch ( $admin_custom_branding->navigation ) {
	case 'login-register':{
		$data_types = array();
		$handle = opendir(TEMPLATEPATH.'/data-types');
		include_once(TEMPLATEPATH.'/data-types/data-type.php');
		while ( false !== ($entry = readdir($handle))) {
			if($entry != '.' && $entry != '..' && $entry != 'data-type.php'){
				include_once(TEMPLATEPATH.'/data-types/'.$entry);
				$tmp = explode('.', $entry);
				$tmp = $tmp[0];
