...
class Content_Types_Admin_Object extends Runway_Admin_Object {
	...
	public function __construct($settings){
		...
		// add actions
		add_action( 'init', array($this, 'add_custom_taxonomies_to_post_types'));
	...
	public function add_custom_taxonomies_to_post_types() {
		if(isset($this->content_types_options['taxonomies']))
		foreach ((array)$this->content_types_options['taxonomies'] as $taxonomy => $values) {
			$content_types_to_adding = array();
			if(isset($this->content_types_options['content_types']))	
			foreach ((array)$this->content_types_options['content_types'] as $content_type => $vals) {
				if(isset($vals['taxonomies']) && in_array($taxonomy, $vals['taxonomies'])){
					$content_types_to_adding[] = $content_type;
				}
			}

			// add taxonomy to post content type
			// $post = isset($this->content_types_options['default_content_types']['post']['taxonomies']) ? 
			// 	$this->content_types_options['default_content_types']['post']['taxonomies'] : array();
			// if(in_array($taxonomy, $post)){
			// 	$content_types_to_adding[] = 'post';
			// }

			// add taxonomy to page content type
			// $page = isset($this->content_types_options['default_content_types']['pages']['taxonomies']) ? 
			// 	$this->content_types_options['default_content_types']['pages']['taxonomies'] : array();
			// if(in_array($taxonomy, $page)){
			// 	$content_types_to_adding[] = 'page';
			// }
	...
	function save_inputs_fields(){
		...
		//$link = home_url().'/wp-admin/options-general.php?page=content-types&navigation=add-inputs&alias='.$form['settings']['alias'];
		$link = home_url().'/wp-admin/admin.php?page=content-types&navigation=add-inputs&alias='.$form['settings']['alias'];
