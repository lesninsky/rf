	...
	(function ($) {
		$(function () {
			...
			function restoreElement(elm){
				...
				apnd.find("span.item-controls").append('<span class="item-type">Default</span>');
			...
			function item_remove($item) {
				...
				if($item.hasClass("menu-spacer-template")) {
					...
					$item.fadeOut('slow', function () {
						...
						$item.find("span.item-controls").append('<a class="item-restore" id="restore-delete-item" title="restore" href="">Restore</a>');
						$item.fadeIn("slow");
					});
				}
			}
