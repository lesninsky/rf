...
if ( ! function_exists( 'send_theme_contact_form' ) ) :
	...
	function send_theme_contact_form() {
		...
		$contact_form_data = $contact_fields->contact_fields_options['defaults'];
		//$contact_fields_data = $contact_form_data['contact_fields'];
		$contact_fields_data = $contact_fields->contact_fields_options['fields'];
		...
		foreach ($commonEmails as $email_field) {
			if (isset($_POST[$email_field]) && simple_email_validate($_POST[$email_field])) {
		...
		$email_body .= $before_h . __('Form Submission', 'framework') . $after_h;
		...
		foreach($_POST as $key => $value){
			if( !in_array($key, $excludeFields) ) {
				...
				// for checkbox values
				if ($info['field_type'] == 'checkbox' && $value == 'on') $value = __('Yes', 'framework');
		...
		if ( array_key_exists('captcha',$_REQUEST )) {	// if (!empty($_REQUEST['captcha'])) {
			if (empty($_SESSION['captcha']) || trim(strtolower($_REQUEST['captcha'])) != $_SESSION['captcha']) {
				$error .= __('Invalid image verification code', 'framework') .'<br />';
			} else {
		...
		if(!$error) {
			...
			$error .= __('Mail delivery error', 'framework') .'<br />';
...
if (!empty($_POST) && isset($_POST['mail_action']) && $_POST['mail_action'] == 'send') {
