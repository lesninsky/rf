<div class="nav-tabs">
	...
	<h2 class="nav-tab-wrapper tab-controlls" style="padding-top: 9px;">
		...
		<!-- <a href="<?php echo $this->self_url('test-connection'); ?>" class="nav-tab <?php if($this->navigation == 'server-settings') {echo "nav-tab-active";} ?>">Test connection</a> -->
	</h2>
		...
		switch($this->action) {			
						...
						switch ($operation) {
							case 'import': {	
								if(isset($_REQUEST['item-id'])){		
								    ...
								    $message = $response['result_message'] . ' on your site';
								}
							} break;
							...
							case 'export': {
								if(isset($_REQUEST['item-id'])){
									...
									$message = $response['result_message'] . ' on server';
								}
							} break;
							...
							case 'import-all': {
							    ...
							    if($success != 0){
							    	$message .= "Imported $success $what. ";
							    }								    
							    if($fail != 0){
							    	$message .= "Failed import $fail $what.";
							    }
							} break;
							...
							case 'export-all': {
							    ...
							    if($success != 0){
							    	$message .= "Exported $success $what. ";
							    }								    

							    if($fail != 0){
							    	$message .= "Failed export $fail $what.";
							    }
							} break;
						}
					}
			...
			case 'delete-connection':{
				if(isset($_GET['connection_id'])){
					...
					$link = home_url().'/wp-admin/options-general.php?page=sync-tool&navigation=connections';
