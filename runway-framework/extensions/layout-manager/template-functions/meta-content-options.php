...
function theme_portfolio_meta_box_content_options () {
	...
	global $layout_manager_admin, $post;
	...
	$layouts  = $layout_manager_admin->get_layouts();
	...
	$layout_alias = get_post_meta($post->ID, 'layout', true);
	if(!empty($layout_alias)) {
		$optional_labels = $layout_manager_admin->get_optional_labels( $layout_alias );

		foreach( $optional_labels as $label_key => $label_value) {
			$optional_label_type = $layout_manager_admin->get_content_elements($label_value['content_type']);
			...
			$optional_label[] = array(
						'name' => __($label_value['label'], 'framework'),
						'desc' => '',
						'id' => 'optional_label',
						'type' => 'select',
						'std' => '',
