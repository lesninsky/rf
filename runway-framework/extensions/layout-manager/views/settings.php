	...
	<table class="form-table">
		<tbody>		
			<tr class="">
				...
				<td>
                    <label><input class="input-check" type="checkbox" value="true" name="content-source-pages" <?php if((isset($settings['content-source-pages']) && $settings['content-source-pages'] === 'true')) { ?>checked<?php } ?>> Enable Pages</label>
				</td>
			</tr>		
			<tr class="">
				<th scope="row" valign="top">
				...
				<td>
					<label><input class="input-check" type="checkbox" value="true" name="content-source-posts" <?php if((isset($settings['content-source-posts']) && $settings['content-source-posts'] === 'true')) { ?>checked<?php } ?>> Enable Posts</label>
				</td>
			</tr>
			<tr class="">
				<th scope="row" valign="top">
				...
				<td>
					<label><input class="input-check" type="checkbox" value="true" name="content-source-sidebars" <?php if((isset($settings['content-source-sidebars']) && $settings['content-source-sidebars'] === 'true')) { ?>checked<?php } ?>> Enable Sidebars</label>
				</td>
			</tr>
<!-- 			<?php $static_block = get_pages(array('post_type' => 'static_block'));
			...
			<?php $custom_post_types = get_post_types(array('_builtin'=>false), 'object'); 
				foreach($custom_post_types as $key => $custom_post_type): 
					$value = isset($custom_post_type->alias) ? $custom_post_type->alias : "";
					...
					<tr class="">
						...
						<td>
							<label><input class="input-check" type="checkbox" value="true" name="content-source-<?php echo $value; ?>" <?php echo (isset($settings["content-source-$value"]) && $settings["content-source-$value"] == 'true') ? 'checked' : ''; ?>> Enable <?php echo ucfirst($custom_post_type_label); ?></label>
						</td>
					</tr>					
				<?php endforeach ?>
		</tbody>
					...
					<div class="postbox">
						...
						<div class="inside" >
							<?php if(isset($layouts_manager->layouts_manager_options['settings']['headers']) ):
								...
								<pre class="code" style="background: #F3F3F3; color: #333; padding: 10px; font-size: 11px; border: 1px solid #EAEAEA;">// Begin Layout
do_action('output_layout','start');</pre>
							...
							<?php if(isset($layouts_manager->layouts_manager_options['settings']['footers']) ):						
								...
								<pre class="code" style="background: #F3F3F3; color: #333; padding: 10px; font-size: 11px; border: 1px solid #EAEAEA;">// End Layout
