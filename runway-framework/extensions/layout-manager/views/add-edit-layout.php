								...
								foreach($custom_post_types as $key => $value) {
									...
									if(count($custom_posts[$value]) > 0) {
										if( isset($layouts_manager->layouts_manager_options['settings']["content-source-$value"]) &&
											...
											<option value="<?php echo $key; ?>"><?php _e(ucfirst($value), 'framework') ?></option>
										<?php endif ?>
									<?php } 
								} ?>								
							...
							<p class="content-type-help-text">
								...
								foreach($custom_post_types as $key => $value) {
									if(count($custom_posts[$value]) > 0) {
										if( isset($layouts_manager->layouts_manager_options['settings']["content-source-$value"]) &&
										 		  $layouts_manager->layouts_manager_options['settings']["content-source-$value"] == true ): ?>
											<span class="description-<?php echo $value; ?>"><?php _e("Choose a $value from as the content source.", 'framework') ?></span>
										<?php endif ?>
									<?php }
								} ?>
