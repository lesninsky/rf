							...
							<div class="handle"></div>
								...
								<select name="content_element_type" class="content-element-type">
									...
									foreach($custom_post_types as $key => $custom_post_type) {
										...
										$custom_posts[$value] = get_posts(array('post_type' => $value));
										if(count($custom_posts[$value]) > 0) {
											if( isset($layouts_manager->layouts_manager_options['settings']["content-source-$value"]) &&
											 		  $layouts_manager->layouts_manager_options['settings']["content-source-$value"] == true ): ?>
								...
								<p class="content-type-help-text">
									...
									foreach($custom_post_types as $key => $custom_post_type) {
										...
										if(count($custom_posts[$value]) > 0) {
											if( isset($layouts_manager->layouts_manager_options['settings']["content-source-$value"]) &&
											 		  $layouts_manager->layouts_manager_options['settings']["content-source-$value"] == true ): ?>
												<span class="description-<?php echo $value; ?>"><?php _e("Choose a $custom_post_type_label from as the content source.", 'framework') ?></span>
											<?php endif ?>
										<?php }
									} ?>
								...
								<div class="secondary-select" style="display: none;">
									...
									<div class="check-override-secondary">
										<label><input class="input-check-secondary" type="checkbox" value="false" name="allow_override"> Allow override on single pages</label>
