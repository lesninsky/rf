...
class Plugin_Installer_Object extends Runway_Object {
	...
	private function get_header_info($plugins_zip_path, $plug_file){
		...
		if($zip->open($plugins_zip_path.$plug_file) ){
			$file_data = $zip->getFromName($info['filename'].'/'.$info['filename'].'.php');				
		}
		$zip->close();	
		...
		if($file_data != ''){
			...
			$plugin_info['slug'] = $info['filename'];
			$plugin_info['file'] = $info['basename'];

			if(file_exists($this->plugins_path . '/'. $plugin_info['slug'] . '/'. $plugin_info['slug'] . '.php') ) {
				
				$file_data = file_get_contents($this->plugins_path . '/'. $plugin_info['slug'] . '/'. $plugin_info['slug'] . '.php');
