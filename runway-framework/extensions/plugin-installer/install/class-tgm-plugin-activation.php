	...
	class TGM_Plugin_Activation {
		...
		public function __construct() {
			...
			$this->strings = array(
				...
				'install_link' 					  => _n_noop( 'Install Now', 'Install Now' ),
			...
			if ( isset( $_POST )) {
				...
				if ( isset( $_POST[sanitize_key( 'plugin_options' )] ) && isset( $_POST[sanitize_key( 'action_save' )] )) {
					...
					if ($action_save) {
						...
						echo("<script>location.href = 'admin.php?page=plugin-installer';</script>");
					}
				}
			}
...
//			if ( isset( $_POST[sanitize_key( 'action' )] ) && 'tgmpa-bulk-install' == $_POST[sanitize_key( 'action' )] && $plugin_table->process_bulk_actions() || $this->do_plugin_install() )
		...
		public function do_plugin_install() {
			...
			if ( isset( $_GET[sanitize_key( 'plugin' )] ) && ( isset( $_GET[sanitize_key( 'tgmpa-install' )] ) && 'install-plugin' == $_GET[sanitize_key( 'tgmpa-install' )] ) ) {
				...
				foreach ( $this->plugins as $plugin ) {
					if ( ! is_plugin_active( $plugin['file_path'] ) ) {
						echo '<p><a href="' . add_query_arg( 'page', $this->menu, admin_url( $this->parent_url_slug ) ) . '" title="' . esc_attr( $this->strings['return'] ) . '" target="_parent">' . rf__($this->strings['return']) . '</a></p>';
						$complete[] = $plugin;
						break;
					}
					/** Nothing to store */
			...
			if ( ! get_user_meta( get_current_user_id(), 'tgmpa_dismissed_notice', true ) ) {
				...
				if ( ! empty( $message ) ) {
					...
					foreach ( $message as $type => $plugin_groups ) {
						...
						$show_install_link  = $install_link ? '<a href="' . add_query_arg( 'page', $this->menu, admin_url( $this->parent_url_general_slug ) ) . '">' . translate_nooped_plural( $this->strings['install_link'], $install_link_count, $this->domain ) . '</a>' : '';
		...
		public function process_bulk_actions() {
			...
			if ( 'tgmpa-bulk-install' === $this->current_action() ) {
				...
				/** Information has been passed via $_GET */
					...
					foreach ( $plugins as $key => $value ) { out($key . "+" . $value);
